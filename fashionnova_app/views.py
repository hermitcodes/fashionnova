...
from .ai_stylist import ai_stylist
...
def wishlist_view(request):
    ...
    categories = Category.objects.filter(
        products__wishlist__user=request.user
    ).annotate(
        count=Count('products__wishlist')
    ).distinct()[:8]
...
def order_detail_view(request, order_id):
    ...
    if request.user.user_type != 'seller':
        ...
        # Sellers can view an order only if it contains at least one of their products.
        try:
...
def add_product_view(request):
    ...
    print(f"DEBUG: add_product_view called with method: {request.method}")
    ...
    if request.method == 'POST':
        ...
        try:
            seller_profile = request.user.sellerprofile
            print(f"DEBUG: Seller profile found: {seller_profile}")
            ...
            print(f"DEBUG: Form data - name: {name}, price: {price}, category: {category_id}")
            ...
            if 'main_image' in request.FILES:
                ...
                product.save()
                print("DEBUG: Product image saved")
            
            messages.success(request, f'Product "{name}" added successfully!')
            print("DEBUG: Success message added, redirecting to products")
...
def seller_orders_view(request):
    ...
    # FIX: Use items__product__seller instead of items__seller
    orders = Order.objects.filter(
        items__product__seller=seller_profile  # Changed this line
    ...
    # Seller-facing status counts for sidebar filters and quick stats
    ...
    context = {
        'orders': orders_page,
        'orders_url_name': 'seller_orders',
        'is_seller_view': True,
        'status_filter': status_filter,
        'total_orders': seller_orders_qs.count(),
        'pending_orders': seller_orders_qs.filter(status='pending').count(),
        'processing_orders': seller_orders_qs.filter(status='processing').count(),
        'shipped_orders': seller_orders_qs.filter(status='shipped').count(),
        'delivered_orders': seller_orders_qs.filter(status='delivered').count(),
        'cancelled_orders': seller_orders_qs.filter(status='cancelled').count(),
        'total_spent': 0,
        'avg_order_value': 0,
        'success_rate': 0,
        'popular_products': [],
...
def update_order_status_view(request, order_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Ensure this seller is authorized to update this order:
            # order must contain at least one item sold by the seller.
            ...
            if new_status == 'delivered' and previous_status != 'delivered' and recipient_email:
                pickup_station = "your selected pickup station"
                send_mail(
                    subject=f"Your order {order.order_number} is ready for pickup",
                    message=(
                        f"Hello {order.user.username},\n\n"
                        f"Great news! Your order {order.order_number} has been delivered.\n"
                        f"Please pick it up from {pickup_station}.\n\n"
                        "Bring your order number and phone number for verification.\n\n"
                        "Thank you for shopping with FashionNova."
                    ),
...
def analyze_product_image_view(request):
    """
    Analyze uploaded product image and return attributes
    """
    if request.method == 'POST' and request.FILES.get('image'):
        try:
            ...
            if not ai_stylist:
                return JsonResponse({
                    'success': False,
                    'error': 'AI stylist service is unavailable'
                })

            # Analyze with AI
            result = ai_stylist.analyze_product_image(image_file)
...
def get_styling_advice_view(request):
    """
    Get styling advice for a product
    """
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            ...
            # 1) {"attributes": {...}} from backend integrations
            # 2) {"product_description": "...", "occasion": "...", "style_preference": "..."} from widget
            product_attributes = data.get('attributes') or {}
            if not product_attributes and data.get('product_description'):
                product_attributes = {
                    'product_type': data.get('product_description', ''),
            ...
            # Get user context from profile (if authenticated)
            user_context = {
                'gender': request.user.gender if request.user.is_authenticated and hasattr(request.user, 'gender') else None,
                'style_preference': data.get('style_preference')
            }

            result = ai_stylist.get_styling_recommendations(product_attributes, user_context)
            ...
            if result['success']:
                ...
                return JsonResponse({
                    'success': False,
                    'error': result.get('error', 'Failed to get advice')
                })
...
@csrf_exempt
def ai_stylist_chat_view(request):
    """
    ...
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            # Widget sends "question"; keep "message" support for API callers.
            ...
            response = ai_stylist.chat_stylist(message, history)
...
def generate_product_description_view(request, product_id):
    """
    Generate AI description for a product (for sellers)
    """
    if request.method == 'POST':
        from .models import Product
        ...
        try:
            ...
            description = ai_stylist.describe_product(product)
